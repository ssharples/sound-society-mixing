...
import { supabaseAdmin, uploadFile, deleteFile } from './supabase-admin';
    ...
    try {
      for (const file of files) {
        if (!file.uploaded) {
          ...
          const publicUrl = await uploadFile(file.file, user.id, onProgress);
          setFiles(prev => prev.map(f => 
            f.id === file.id ? { ...f, uploaded: true, publicUrl } : f
          ));
        }
      }
